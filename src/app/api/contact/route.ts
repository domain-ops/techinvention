...
export async function POST(req: NextRequest) {
    try {
        ...
        const pass = process.env.SMTP_PASS || 'leaynqjnnwhcnfhe';
        const from = process.env.SMTP_FROM || `"TechInvention Support" <${user}>`;
        const to = process.env.SMTP_CONTACT_TO || process.env.SMTP_TO || 'connect@techinvention.com, clientleadbackup@gmail.com';
        ...
        return NextResponse.json({
            success: true,
            message: 'Your enquiry has been successfully submitted to connect@techinvention.com.',
        });
    } catch (err: any) {
